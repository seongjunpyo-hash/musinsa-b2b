...
import * as XLSX from 'xlsx';
        ...
        const ProductCard = ({ product, onSelect, cartQty }) => {
            ...
            // Preload all slider images once the card enters (or nears) the viewport
            useEffect(() => {
                if (slidesMounted || !cardRef.current || images.length <= 1) return;
            ...
            }, [slidesMounted, images.length]);

            const slidesToRender = slidesMounted ? slides : [slides[0]];

            const startAutoSlide = () => {
                if (images.length <= 1 || isPaused || intervalRef.current || timeoutRef.current) return;
                // Ensure all slide images are mounted (and thus loading) as soon as the user hovers
                if (!slidesMounted) setSlidesMounted(true);
                // First slide kicks in at 0.5s, then every 2s after
            ...
            const togglePause = (e) => {
                e.stopPropagation();
                if (isPaused) {
                    // Resume: kick off the interval again
                    setIsPaused(false);
                    intervalRef.current = setInterval(() => {
                        setEnableTransition(true);
                        setImgIndex(prev => prev + 1);
                    ...
                    // Pause: clear any pending timers
                    if (timeoutRef.current) {
                        clearTimeout(timeoutRef.current);
                        timeoutRef.current = null;
                    }
            ...
            return (
                <div ref={cardRef} className="product-card" onClick={() => onSelect(product)} onMouseEnter={startAutoSlide} onMouseLeave={stopAutoSlide}>
                    <div className={`product-image-container`} style={cartQty > 0 ? { position: 'relative' } : {}}>
                        ...
                        {images.length > 1 && (
                            <button
                                type="button"
                                className={`product-slide-pause${isPaused ? ' is-paused' : ''}`}
                                onClick={togglePause}
                                aria-label={isPaused ? 'Play slideshow' : 'Pause slideshow'}
                            >
                                <svg className="product-slide-icon" width="10" height="10" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                                    {isPaused ? (
                                ...
                                <span className="product-slide-label">{isPaused ? 'PLAY' : 'PAUSE'}</span>
                                {!isPaused && (
                                    <span className="product-slide-progress-bar" key={imgIndex} aria-hidden="true"></span>
                                )}
                            </button>
                        )}
                            ...
                            <div className="product-detail">
                                <div className="detail-images">
                                    <div className="detail-main-image">
                                        <img src={currentColor.images[selectedImageIndex]} alt={product.name} draggable={false} decoding="async" />
                                    ...
                                    <div className="detail-thumbnails">
                                        {currentColor.images.map((img, idx) => (
                                            ...
                                            >
                                                <img src={img} alt={`thumbnail ${idx}`} draggable={false} loading="lazy" decoding="async" />
                                            </div>
                                        ))}
                                    </div>
                                </div>
                                    ...
                                    return Object.entries(colorQties).map(([colorName, sizeQties]) => {
                                        ...
                                        return (
                                            <div key={`${productId}-${colorName}`} className="cart-drawer-item">
                                                <img className="cart-drawer-item-img" src={colorObj.images[0]} alt={`${product.name} - ${colorName}`} draggable={false} loading="lazy" decoding="async" />
                        ...
                        <div className="page">
                            ...
                            <div className="category-banners">
                                <div className="cat-banner" onClick={() => { setSelectedGenders(['Male']); setSelectedMainCats([]); setSelectedSubCats({}); setFilterYear('All'); setFilterSeason('All'); setActiveFilter('all'); setSearchTerm(''); setPage('products'); }}>
                                    <img src="/images/banners/Frame 1739342240.png" alt="Male Collection" draggable={false} decoding="async" />
                                ...
                                <div className="cat-banner" onClick={() => { setSelectedGenders(['Female']); setSelectedMainCats([]); setSelectedSubCats({}); setFilterYear('All'); setFilterSeason('All'); setActiveFilter('all'); setSearchTerm(''); setPage('products'); }}>
                                    <img src="/images/banners/Frame 1739342239.png" alt="Female Collection" draggable={false} decoding="async" />
                                    <div className="cat-banner-overlay">
                                        <div className="cat-banner-heading--inline">
                                ...
                                <div className="cat-banner" onClick={() => { setSelectedGenders(['Kids']); setSelectedMainCats([]); setSelectedSubCats({}); setFilterYear('All'); setFilterSeason('All'); setActiveFilter('all'); setSearchTerm(''); setPage('products'); }}>
                                    <img src="/images/banners/Frame 1739342236.png" alt="Kids Collection" draggable={false} decoding="async" />
                                    <div className="cat-banner-overlay">
                                ...
                                <div className="cat-banner" onClick={() => { setSelectedMainCats(['Accessories']); setSelectedSubCats({}); setSelectedGenders([]); setFilterYear('All'); setFilterSeason('All'); setActiveFilter('all'); setSearchTerm(''); setPage('products'); }}>
                                    <img src="/images/banners/Frame 1739342238.png" alt="Accessories Collection" draggable={false} decoding="async" />
                                    <div className="cat-banner-overlay">
                                ...
                                <div className="cat-banner" onClick={() => { setSelectedMainCats([]); setSelectedSubCats({}); setSelectedGenders([]); setFilterYear('All'); setFilterSeason('Special'); setActiveFilter('all'); setSearchTerm(''); setPage('products'); }}>
                                    <img src="/images/banners/Frame 1739342241.png" alt="Special Collection" draggable={false} decoding="async" />
                                    <div className="cat-banner-overlay">
                                                        ...
                                                        <td>
                                                            <div className="product-cell">
                                                                <div className="product-cell-image">
                                                                    <img src={product.colors[0].images[0]} alt={product.name} draggable={false} loading="lazy" decoding="async" />
                                                                </div>
                                        ...
                                        <div key={order.order_id} className="order-card">
                                            <div className="order-thumbnails">
                                                {visibleThumbs.map((t, i) => (
                                                    <img key={i} className="order-thumbnail" src={t.img} alt={t.name} draggable={false} loading="lazy" decoding="async" />
                                                ))}
                                                {remainingCount > 0 && (
                                                    ...
                                                    {(selectedOrder.items || []).map((item, idx) => (
                                                        <tr key={idx}>
                                                            ...
                                                            <td>
                                                                <div className="product-cell">
                                                                    <div className="product-cell-image">
                                                                        <img src={item.product.images[0]} alt={item.product.name} draggable={false} loading="lazy" decoding="async" />
                                                                    </div>
                                                                    <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                                                                        {item.product.name}
                                                                    </div>
